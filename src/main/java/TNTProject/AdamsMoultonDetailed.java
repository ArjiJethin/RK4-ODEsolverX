...
public class AdamsMoultonDetailed {
    ...
    static void adamsMoulton(double x0, double y0, double xEnd, double h, Expression expression) {
        int n = (int) ((xEnd - x0) / h);
        ...
        System.out.println("n\tx_n\tStarting y_n\tPredicted y*_n\tCorrected y_n");
        System.out.printf("%d\t%.1f\t%.6f\t-\t-\n", 0, x[0], y[0]);
        ...
        for (int i = 3; i < n; i++) {
            ...
            double yCorrected = y[i] + h / 24 * (9 * fPredict + 19 * f[i] - 5 * f[i - 1] + f[i - 2]);

            x[i + 1] = x[i] + h;
            f[i + 1] = f(x[i + 1], yCorrected, expression);
            y[i + 1] = yCorrected;

            System.out.printf("%d\t%.1f\t%.6f\t%.6f\t%.6f\n", i + 1, x[i + 1], y[i], yPredict, yCorrected);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            adamsMoulton(x0, y0, xEnd, h, expression);
